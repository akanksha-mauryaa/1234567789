  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="glass p-5 rounded-xl border border-border hover:bg-surface/50 transition-colors">
          <div className="text-[10px] text-muted font-mono tracking-wider">TYPE</div>
          <div className="text-3xl font-mono mt-3 font-bold text-accent uppercase">{item.file_type || 'unknown'}</div>
        </div>
        <div className="glass p-5 rounded-xl border border-border hover:bg-surface/50 transition-colors">
          <div className="text-[10px] text-muted font-mono tracking-wider">STATUS</div>
          <div className={`text-3xl font-mono mt-3 font-bold ${item.status === 'completed' ? 'text-neon' : 'text-danger'} uppercase`}>
            {item.status || '—'}
          </div>
        </div>
        <div className="glass p-5 rounded-xl border border-border hover:bg-surface/50 transition-colors">
          <div className="text-[10px] text-muted font-mono tracking-wider">SAFE?</div>
          <div className={`text-3xl font-mono mt-3 font-bold ${safe ? 'text-neon' : 'text-danger'} uppercase`}>
            {safe ? 'YES' : 'NO'}
          </div>
        </div>
        <div className="glass p-5 rounded-xl border border-border hover:bg-surface/50 transition-colors">
          <div className="text-[10px] text-muted font-mono tracking-wider">LABEL COUNT</div>
          <div className="text-3xl font-mono mt-3 font-bold text-accent2 uppercase">{item.label_count ?? '0'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass p-6 rounded-xl border border-border">
          <h3 className="text-[10px] font-mono tracking-widest text-muted mb-4 uppercase">// Detected Labels</h3>
          <div className="bg-surface/30 p-4 rounded-lg border border-border/50 min-h-[100px]">
            <LabelTags labels={item.labels} maxVisible={10} />
          </div>
        </div>

        {/* Confidence Breakdown Bars */}
        <div className="glass p-6 rounded-xl border border-border">
          <h3 className="text-[10px] font-mono tracking-widest text-muted mb-4 uppercase">// Confidence Breakdown</h3>
          <div className="space-y-4">
            {topLabels.length > 0 ? topLabels.map((l, i) => (
              <div key={i}>
                <div className="flex justify-between text-[10px] mb-1 font-mono">
                  <span className="text-primary uppercase tracking-tight">{l.name}</span>
                  <span className="text-accent">{l.confidence}%</span>
                </div>
                <div className="w-full h-1.5 bg-surface rounded-full overflow-hidden border border-border/50">
                  <div 
                    className={`h-full transition-all duration-1000 ${l.confidence > 90 ? 'bg-neon shadow-[0_0_8px_rgba(0,255,136,0.5)]' : 'bg-accent'}`} 
                    style={{ width: `${l.confidence}%` }} 
                  />
                </div>
              </div>
            )) : (
              <div className="text-[10px] text-muted font-mono italic">No confidence data available for this file type.</div>
            )}
          </div>
        </div>
        ...
        {!safe && modDetails.length > 0 && (
          <div className="glass p-6 rounded-xl border border-danger/30 md:col-span-2 bg-danger/5">
            <h3 className="text-[10px] font-mono tracking-widest text-danger mb-4 uppercase">// Moderation Alerts Detected</h3>
            <div className="flex flex-wrap gap-3">
              {modDetails.map((m, i) => (
                <div key={i} className="flex flex-col gap-1 bg-surface/50 p-3 rounded-lg border border-danger/20 min-w-[140px]">
                  <span className="text-[10px] font-mono text-muted uppercase">Issue</span>
                  <span className="text-xs font-bold text-danger">{m.name}</span>
                  <span className="text-[9px] font-mono text-muted">Confidence: {m.confidence}%</span>
                </div>
              ))}
            </div>
          </div>
